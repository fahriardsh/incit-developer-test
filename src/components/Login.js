...
function Login() {
    ...
    const handleClickLogin = () => {
      let checkEmail = "@";
      let checkPassword = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&#~])[A-Za-z\d@$!%*?&#~]{8,}$/;
        if(email.match(checkEmail) && password.match(checkPassword)){
          let ePassword = bcrypt.genSaltSync(10)
          localStorage.setItem('email', JSON.stringify(email));
          localStorage.setItem('password', JSON.stringify(ePassword));
  ...
  return (
    <div className="Login">
        <Grid item xs={12} style={{textAlign: 'left', margin: 8}}>
          <img src={popshop}></img>
        </Grid>
        <section className="Login-section">
          <Grid container spacing={2}>
            ...
            <Grid item xs={6} style={{textAlign: 'center'}}>
              ...
              <br></br>
              <br></br>
              <button className='Login-btn' onClick={handleClickLogin}>Login</button>
              <br></br>
              <br></br>
              <br></br>
